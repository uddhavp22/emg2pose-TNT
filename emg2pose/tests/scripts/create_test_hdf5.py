...
def create_a_sample_hdf5_file():
    ...
    timeseries_dtype = np.dtype(
        [("time", "<f8"), ("joint_angles", "<f8", (20,)), ("emg", "<f4", (16,))]  # Still 16 channels in HDF5; downsampling applied during training
    )
    main_timeseries = np.array(np.empty((T,), dtype=timeseries_dtype))

    main_timeseries["time"] = np.arange(T, dtype="<f8")
    main_timeseries["emg"] = np.random.randn(T, 16)  # Still 16 channels; downsampling applied during training
    ...
    metadata = {
        ...
        "side": "right",
        "sample_rate": 2000.0,
        "num_channels": 16,  # Raw HDF5 channels; downsampling applied during training
